...
func (df *Diff) consume(ctx context.Context, rangeInfo *splitter.RangeInfo) bool {
	...
	if err != nil {
		...
		log.Debug("checksum failed", zap.Any("chunk id", rangeInfo.ChunkRange.Index), zap.Int64("upstream chunk size", upCount), zap.Int64("downstream chun size", downCount), zap.String("table", df.workSource.GetTables()[rangeInfo.GetTableIndex()].Table))
		state = checkpoints.FailedState

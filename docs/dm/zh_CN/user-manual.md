...
2. 阅读并且了解 [配置文件](./configuration) 章节
...
6. 学习 [任务管理](./task-handling) 章节来管理和查看任务的运行
...
如果使用 dm-ansible 安装，可以在 `<path-to-dm-ansible>/conf` 找到下面任务配置文件样例
...
| 概念         | 解释                                                         | 配置文件                                                     |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| instance-id  | 唯一确定一个 MySQL / MariaDB 实例（ansible 部署会用 host:port 来组装成该 ID） | dm-master.toml 的 `mysql-instance`;<br> task.yaml 的 `instance-id` |
| dm-worker ID | 唯一确定一个 dm-worker （取值于dm-worker.toml 的 `worker-addr` 参数） | dm-worker.toml 的 `worker-addr`;<br> dmctl 命令行的 `-worker` / `-w` flag  |
...
上游数据库实例表的黑名过滤名单规则。过滤规则类似于 MySQL replication-rules-db / tables, 可以用来过滤或者只同步某些 database 或者某些 table 的所有操作。详情见 [Task 配置项介绍](./configuration/argument-explanation.md)

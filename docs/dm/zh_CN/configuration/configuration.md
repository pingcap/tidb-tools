...
样例: [task.yaml](./task.yaml)

[task.yaml](./task.yaml) 包含下面 [全局配置] 和 [instance 配置] 两部分
...
主要包含下面几个功能配置集，格式如（e.g. routes）-> map[rule name : route rule config ] ，详情见 [Task 配置项介绍]
...
black-white-list:		# 该上游数据库实例的匹配的表的黑名过滤名单规则集。
    instance:			# 具体解释见 `Task 配置项介绍` 文档的 `black white list rule`
    ​    do-dbs: ["~^test.*", "do"]
...
column-mappings:        # 上游数据库实例的匹配的表的 column 映射规则集。
    instance-1:	    	# 限制和具体解释见 `Task 配置项介绍` 文档的 `column mapping rule`
    ​    schema-pattern: "test_*"
    ​    table-pattern: "t_*"
...
syncers:                     			# syncer 组件运行配置参数
    ...
    ​    batch: 1000				       	# syncer 同步到下游数据库的一个事务批次
    ​    max-retry: 100			       	# syncer 同步到下游数据库出错的事务的重试次数
```

### Task 的 MySQL Instances 配置

定义具体的数据同步子任务，支持从单个或者多个上游 MySQL 实例同步到一个下游数据库实例
...
mysql-instances:
    ...
    ​    meta:         # 下游数据库的 checkpoint 不存在， meta 为 binlog 同步开始的位置; 否则没有作用 
    ​      binlog-name: binlog-00001
    ​      binlog-pos: 4

    ​    route-rules: ["user-route-rules-schema", "user-route-rules"]       # 该上游数据库实例的匹配的表到下游数据库的映射规则名称。     具体配置见全局配置的功能配置项集的 routes
    ​    filter-rules: ["user-filter-1", "user-filter-2"]                      # 该上游数据库实例的匹配的表的 binlog event 过滤规则名称。 具体配置见全局配置的功能配置项集的 filters
    ​    column-mapping-rules: ["instance-1"]                               # 该上游数据库实例的匹配的表的 column 映射规则名称。 具体配置见全局配置的功能配置项集的 column-mappings
    ​    black-white-list:  "instance"                                      # 该上游数据库实例的匹配的表的黑名过滤名单规则名称。 具体配置见全局配置的功能配置项集的 black-white-list

    ​    mydumper-config-name: "global" 			# mydumper 配置名称，具体配置见全局配置的功能配置项集的 mydumpers （不能和 mydumper 同时设置）
    ​    loader-config-name: "global"    			# loader 配置名称，具体配置见全局配置的功能配置项集的 loaders （不能和 loader 同时设置）
    ​    syncer-config-name: "global" 			# syncer 配置名称，具体配置见全局配置的功能配置项集的 syncers （不能和 syncer 同时设置）
    ...
    ​      mydumper-path: "./mydumper"           # 见下文 mydumpers 配置项说明
    ​      threads: 4
    ​      chunk-filesize: 8
    ​      skip-tz-utc: true
    ...
    ​      pool-size: 32	     # 见下文 loaders 配置项说明
    ​      dir: "./dumped_data"
    
    ​    syncer:		# syncer 相关配置（不能和 syncer-config-name 同时设置）
    ​      worker-count: 32	# 见下文 syncers 配置项说明
    ​      batch: 2000
    ​      max-retry: 200
```

...
说明：表示 relay log 模块 在 dm-worker 内部遇到错误并且退出了。
...
说明：表示对应 instance 上面 relay log 文件损坏的个数。
...
说明：表示 relay 模块从上游的 mysql 读取 binlog 时遇到的错误数。
...
说明：表示 relay 模块写 binlog 到磁盘时遇到的错误数。
...
说明：此项 metric 表明 relay log 最新的文件序列号。如 value = 1 表示 relay-log.000001。也代表已经从上游 MySQL 拉取的最大 binlog 文件序号
...
说明：此项 metric 表明 relay 与上游 master 相比落后的 binlog file 个数。
...
说明：此项 metric 表明 relay log 的 position 移动情况。由于文件会 rotate，所以会出现数字骤减重新增长的情况。
...
说明：loader 模块在 dm-worker 内部遇到错误并且退出了。需要告警。 
...
说明：表示 loader 导入的全量数据中 table 的数量总和。

#### data file count

query: `dm_loader_data_file_count{task="$task",instance="$instance"}`

说明：表示 loader 导入的全量数据中 数据文件（内含 `INSERT INTO` 语句）的数量总和。
...
说明：预计有多少分钟可以和 master 完全同步。单位: 分钟
...
说明：表示 syncer 模块 在 dm-worker 内部遇到错误并且退出了。
...
说明：此项 metric 表明 syncer 与上游 master 相比落后的 binlog file 个数
...
说明：此项 metric 表明 syncer 与 relay 相比落后的 binlog file 个数
...
说明：syncer 执行 transaction 到下游的耗时

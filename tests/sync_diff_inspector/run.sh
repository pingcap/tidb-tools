...
check_contains "will split chunk by random again" ./output/sync_diff.log
mysql -uroot -h 127.0.0.1 -P 4000 -e "analyze table diff_test.test"
sync_diff_inspector --config=./config_base.toml > $OUT_DIR/diff.output
check_contains "check pass!!!" $OUT_DIR/diff.output
check_not_contains "will split chunk by random again" ./output/sync_diff.log
rm -f ./output/sync_diff.log
...
sed 's/"127.0.0.1"#MYSQL_HOST/${MYSQL_HOST}/g' ./config_base_mysql.toml | sed -i 's/3306#MYSQL_PORT/${MYSQL_PORT}/g' > ./config_base_mysql_.toml
